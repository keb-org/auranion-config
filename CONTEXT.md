...
## Reasoning-effort contracts (API-verified via 9router invalid-value probes)
...
- `inferenceModels`: six entries `{ name: desktop_alias, labelOverride: desktop_label, supports1m }`
...
- Custom model option env describing the six-model catalog.
...
- `~/.codex/desktop-model-providers.json`: `version:1`, `default_provider:"openai"`, providers `openai` + `auranion`, and `model_providers` mapping all six slugs to `auranion`.
